...
VBFILTER_DIR=$(cd ${SCRIPT_DIR}/../doxygen-vb-filter && pwd) # vbfilter ディレクトリの絶対パス
HEADER="${DOC_SH}: "                                         # シェルスクリプトから表示するメッセージのヘッダ
...
if [ ! -e ${SRC_DIR}/$1/Doxyfile ]; then
	...
	sed -i -r "s/INPUT_FILTER\s*=.+/INPUT_FILTER           = \"gawk -f ${VBFILTER_DIR//\//\\\/}\/vbfilter.awk\"\n/g" ${SRC_DIR}/$1/Doxyfile
fi

cd ${SRC_DIR}/$1 && doxygen
